...
def get_products(db: SessionLocal = Depends(get_db), current_user = Depends(get_current_user)) -> List[Product]:
    """Return all products."""
...
def get_product_by_id(id:int = Path(ge=1), db: SessionLocal = Depends(get_db), current_user = Depends(get_current_user)) -> Product:
...
def get_products_by_category(category:str = Query(min_length=3, max_length=15), db: SessionLocal = Depends(get_db), current_user = Depends(get_current_user)) -> List[Product]:
...
def register_product(product:Product, db: SessionLocal = Depends(get_db), current_user = Depends(get_current_user)) -> dict:
